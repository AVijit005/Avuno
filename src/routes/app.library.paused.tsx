    ...
    >
      {items.length === 0 ? (
        <EmptyState
          ...
          description="You don't have any stories waiting for the right time."
        />
      ) : (
              ...
              <div key={m.id} className="glass flex items-center gap-4 rounded-2xl p-3">
                ...
                <div className="flex shrink-0 flex-col gap-1.5">
                  <Link
                    to="/app/media/$id"
                    params={{ id: m.id }}
                    className="press-scale inline-flex items-center justify-center gap-1.5 rounded-full bg-gradient-to-r from-primary to-secondary min-h-[44px] px-4 text-[13px] font-medium text-primary-foreground focus-ring"
                  >
                    <Play className="h-4 w-4 fill-current" /> Resume
                  </Link>
                  <button
                    className="press-scale glass-subtle inline-flex items-center justify-center gap-1.5 rounded-full min-h-[44px] px-4 text-[13px] font-medium text-muted-foreground hover:text-foreground focus-ring"

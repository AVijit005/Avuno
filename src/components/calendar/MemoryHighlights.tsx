...
export function MemoryHighlights({ highlights: propHighlights }: Props) {
  const highlights = propHighlights?.length
    ? propHighlights.map((h) => ({
        ...h,
        media: { backdrop: h.media.poster, poster: h.media.poster },
      }))
    : [];
...
function BentoCard({
  ...
  h: {
    ...
    value: string;
    note: string;
    media: { backdrop?: string | null; poster?: string | null };
  };
  colSpan: string;
  index: number;
}) {
  ...
  return (
    ...
    >
      <motion.img
        src={(h.media.backdrop ?? h.media.poster ?? "") as string}
        alt=""
        className="absolute inset-0 h-full w-full object-cover object-center"
        animate={{
          scale: hovered ? 1.08 : 1,
          filter: hovered ? "brightness(0.75) saturate(1.2)" : "brightness(0.45) saturate(0.7)",
        }}
        ...
        <div className="relative h-full px-5 py-4 flex flex-col justify-between">
          <div className="flex items-center gap-2">
            <motion.span
              ...
              animate={{
                boxShadow: hovered
                  ? "0 0 10px 3px oklch(0.72 0.18 255 / 0.7)"
                  : "0 0 4px 1px oklch(0.72 0.18 255 / 0.3)",
              }}

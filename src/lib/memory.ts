...
// Source of truth for memory data, types, and selectors across Avuno.
...
export function buildMemories(items: MediaItem[]): Record<string, MediaMemory | null> {
  return Object.fromEntries(items.map((m) => [m.id, buildMemoryItem(m)] as const));
}

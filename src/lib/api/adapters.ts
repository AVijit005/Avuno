...
export function mediaResponseToMediaItem(m: MediaResponse): MediaItem {
  return {
    ...
    year: m.releaseYear ?? ((m as any).releaseDate ? new Date((m as any).releaseDate).getFullYear() : new Date().getFullYear()),
...
export function libraryItemToMediaItem(item: LibraryItemResponse): MediaItem {
  ...
  return {
    ...
    year: media?.releaseYear ?? ((media as any)?.releaseDate ? new Date((media as any).releaseDate).getFullYear() : new Date().getFullYear()),
...
export function continueItemToMediaItem(item: ContinueItem): MediaItem {
  return {
    ...
    title: item.title,
    kind: MEDIA_TYPE_TO_KIND[item.mediaType] ?? "movie",
    year: (item as any).releaseYear ?? ((item as any).releaseDate ? new Date((item as any).releaseDate).getFullYear() : new Date().getFullYear()),
